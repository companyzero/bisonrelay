...
var (
	...
	// SuggestedKXLogMsg is the message stored in logs about suggested KXs.
	SuggestedKXLogMsg = "Suggested KX to %s %q"

	// suggestedKXRegexp is a Regexp that matches the log messages about
	// suggested KX.
	//
...
func (db *DB) readLogMsg(logFname string, pageSize, pageNum int) ([]PMLogEntry, error) {
	...
	msgs := loggedMessages[pageStart:pageEnd]
...
func (db *DB) ReadLogPM(tx ReadTx, uid UserID, page, pageNum int) ([]PMLogEntry, error) {
	...
	msgs, err := db.readLogMsg(logFname, page, pageNum)
	if err != nil {
		return nil, err
	}

	// Post-Process messages after decoding them.
	for i := range msgs {
		if msgs[i].Internal {
			suggestKxMatch := suggestedKXRegexp.FindStringSubmatch(msgs[i].Message)
			if len(suggestKxMatch) == 3 {
				var targetUID UserID
				if err := targetUID.FromString(suggestKxMatch[1]); err == nil {
					if !db.HasKXSuggestion(tx, uid, targetUID) {
						// Already accepted or declined
						// this KX suggestion, so
						// replace message.
						msgs[i].Message = fmt.Sprintf("Processed KX suggestion to %s (%s)",
							targetUID, suggestKxMatch[2])
